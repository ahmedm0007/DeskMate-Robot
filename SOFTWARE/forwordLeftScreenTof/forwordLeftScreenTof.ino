...
bool tofStopCondition() {
  ...
  return (measure.RangeMilliMeter > 200);
}
...
void driveDistance(float distanceCM, int pwm) {
  ...
  while (true) {
    ...
    if (tofStopCondition()) {
      ...
      display.println("TOF > 200 mm");
      display.display();

      Serial.println("Stopped by TOF");
